...
if (localID.length<=1)
	...
	$('input[type=radio][id=radio-en]').prop('checked', true); 
	}
else
	{
...
if (localID!=""){
			...
			$('input[type=radio][id=radio-zh-hans]').prop('checked', true);
			//$("#boxLabel0").text("语言");
			$("#boxLabel1").text("省");
			$("#boxLabel3").text("历代");
			$("#boxLabel2").text("名胜");
			...
			$('input[type=radio][id=radio-zh-hant]').prop('checked', true);
			//$("#boxLabel0").text("語言");
			$("#boxLabel1").text("省");
			$("#boxLabel3").text("歷代");
			$("#boxLabel2").text("名勝");
...
//            success: parseImageXml
		...
		$(this).find("binding[name='gqURL']").each(function() {
		    tableRow=tableRow+"<div class='thumb-images'><a target='_blank' href='"+$(this).find("uri").text() + "'>";
		});
	
		// pull the "binding" element that has the name attribute of "thumbURL"
		$(this).find("binding[name='thumbURL']").each(function() {
		    tableRow=tableRow+"<span class='helper'></span><img src='"+$(this).find("uri").text() + "'></a></div>";
		});     
	    });
...
$(document).ready(function(){
	...
	//$("#box0").change(function(){
		 $('input[type=radio][name=language_selected]').change(function() {
			// searching, so show the spinner icon
			$('#searchSpinner').show();
			//var isoLanguage= $("#box0").val();
			var isoLanguage= $(this).val();
			//if (isoLanguage=='en') {
			if (this.value == 'en') {
			//$("#boxLabel0").text("Language");
			$("#boxLabel1").text("Province");
	...
	$("#box1").change(function(){
			...
			$('#searchSpinner').show();
			//var isoLanguage= $("#box0").val();
			var isoLanguage= $('input[type=radio][name=language_selected]:checked').val();
			var provinceSelection = $("#box1").val();
			var dynastySelection = $("#box3").val();
			setSiteOptions(provinceSelection,dynastySelection,isoLanguage);
	});
...
//	setCategoryOptions();
	...
	$("#resetButton").click(function(){
			...
			var isoLanguage= $('input[type=radio][name=language_selected]:checked').val();
			window.location.replace("http://tcadrt.org/tool?"+isoLanguage);
...
function parseXml(xml) {
    ...
    $('#searchSpinner').hide();
    //isoLanguage= $("#box0").val();
	var laguageLabel = $('input[type=radio][name=language_selected]:checked').val();
	if (laguageLabel=='en') {
		siteLabel="Historic Site";
		buildingLabel="Historic Object";
		}
	else if (laguageLabel=='zh-hans') {
		siteLabel="名胜 ";
		buildingLabel="文物";
		}
	else {
    ...
    if (numResults < 1) {
        ...
        $("#div2").html("<h4 class=\"text-success\">Found "+numResults+" Sites</h4>");

        //step through each "result" element
        $(xml).find("result").each(function() {

            tableRow='<div class="result_row">';
		    ...
		    tableRow = tableRow + longitude +'&amp;t=h&amp;z=16&amp;key=AIzaSyCkFFHwW14kMU-j1_BYtFBMnx2gv0jwu3M">Open location in map application</a><br/>';
		    tableRow = tableRow + '<img src="http://maps.googleapis.com/maps/api/staticmap?center='+latitude+','+longitude+'&amp;zoom=11&amp;size=300x300&amp;markers=color:green%7C'+latitude+','+longitude+'&amp;sensor=false&amp;key=AIzaSyCkFFHwW14kMU-j1_BYtFBMnx2gv0jwu3M"/>'
		    tableRow = tableRow + '<img src="http://maps.googleapis.com/maps/api/staticmap?center='+latitude+','+longitude+'&amp;maptype=hybrid&amp;zoom=18&amp;size=300x300&amp;markers=color:green%7C'+latitude+','+longitude+'&amp;sensor=false&amp;key=AIzaSyCkFFHwW14kMU-j1_BYtFBMnx2gv0jwu3M"/><br/>'
